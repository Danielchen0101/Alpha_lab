...
// 2. 1 Week: 生成交易日日期标签 - 每个交易日只显示一个日期标签
...
// 3. 1 Month: 生成智能日期标签 - 选择关键日期节点
const getProfessional1MonthTicks = (chartData: ChartDataPoint[]): string[] => {
  if (!chartData || chartData.length === 0) {
    return [];
  }
  
  console.log('[1 Month X轴] ====== 生成智能日期标签 (选择关键日期节点) ======');
  console.log('[1 Month X轴] 输入数据点数量:', chartData.length);
  ...
  console.log(`[1 Month X轴] 唯一交易日数量: ${uniqueDates.length}`);
  
  // 3. 智能选择关键日期标签（大约6-8个标签）
  const ticks: string[] = [];
  
  if (uniqueDates.length <= 8) {
    // 如果交易日不多，全部显示
    console.log('[1 Month X轴] 交易日较少，显示所有日期');
    uniqueDates.forEach(item => {
      ticks.push(item.dataPoint.date);
    });
  } else {
    // 智能选择关键日期：开始、结束、以及中间的几个关键点
    const totalDays = uniqueDates.length;
    
    // 总是包含第一个日期
    ticks.push(uniqueDates[0].dataPoint.date);
    console.log(`[1 Month X轴] 添加第一个日期: ${uniqueDates[0].dateKey}`);
    ...
    for (let i = 1; i < targetLabelCount - 1; i++) {
      ...
      if (index > 0 && index < totalDays - 1) {
        ...
        console.log(`[1 Month X轴] 添加中间日期 ${i}: ${uniqueDates[index].dateKey} (索引: ${index})`);
      }
    }
    
    // 总是包含最后一个日期
    ticks.push(uniqueDates[totalDays - 1].dataPoint.date);
    console.log(`[1 Month X轴] 添加最后一个日期: ${uniqueDates[totalDays - 1].dateKey}`);
  }
  
  // 4. 确保ticks按时间排序（从左到右：旧 -> 新）
  ticks.sort((a, b) => new Date(a).getTime() - new Date(b).getTime());
  
  console.log(`[1 Month X轴] 生成${ticks.length}个智能日期标签`);
  
  // 5. 打印最终选择的日期
  console.log('[1 Month X轴] 最终选择的日期标签:');
  ticks.forEach((tick, index) => {
    const date = new Date(tick);
    const month = date.getUTCMonth() + 1;
    const day = date.getUTCDate();
...
const SymbolAnalysis: React.FC = () => {
  ...
  const loadHistoricalPrices = async () => {
    ...
    try {
        ...
        if (item.time) {
          try {
            ...
            if (item.time.includes(' ')) {
              // 分钟线格式: "2026-02-17 15:30:00" 
              // Twelve Data API返回的是纽约时间（America/New_York）
              // 我们需要明确指定时区为纽约时间
              // 方法：添加时区偏移 "-05:00"（EST）或 "-04:00"（EDT）
              // 注意：当前是2026年3月22日，美国已进入夏令时（EDT）
              // EDT时区偏移：UTC-4
              timeStr = item.time.replace(' ', 'T') + '-04:00';
            } else {
              // 日线格式: "2026-02-17" -> "2026-02-17T00:00:00-04:00" (EDT)
              timeStr = item.time + 'T00:00:00-04:00';
            }
            ...
            if (!isNaN(date.getTime())) {
              // 成功解析time字段
              console.log(`[Analyze] 转换 ${index+1}: 使用time字段 "${item.time}" -> ${date.toISOString()} (纽约时间)`);
              return {
      ...
      // === 为Day图添加16:00占位点 ===
      let finalChartData = chartDataWithIndicators;
      if (selectedTimeframe === '1D' && chartDataWithIndicators.length > 0) {
        console.log(`[Analyze] === 为Day图添加16:00占位点 ===`);
        ...
        const lastMinuteUTC = lastDate.getUTCMinutes();
        const isAlready1600 = lastHourUTC === 20 && lastMinuteUTC === 0; // 20:00 UTC = 16:00 EDT
        
        if (!isAlready1600) {
          // 创建16:00占位点
          const placeholderPoint = {
            date: endTimeUTC.toISOString(),
            open: lastDataPoint.close, // 使用最后一个收盘价作为开盘价
            high: lastDataPoint.close, // 高低价都等于收盘价
            low: lastDataPoint.close,
            close: lastDataPoint.close, // 收盘价不变
            volume: 0, // 交易量为0
            sma20: lastDataPoint.sma20 || undefined,
            sma50: lastDataPoint.sma50 || undefined,
            ema12: lastDataPoint.ema12 || undefined,
            ema26: lastDataPoint.ema26 || undefined,
            rsi: lastDataPoint.rsi || undefined
          };
          
          // 添加到数据末尾
          finalChartData = [...chartDataWithIndicators, placeholderPoint as any];
          console.log(`[Analyze] 添加16:00占位点: ${endTimeUTC.toISOString()} (纽约时间16:00)`);
          console.log(`[Analyze] 占位点价格: ${lastDataPoint.close}`);
          console.log(`[Analyze] 数据条数: ${chartDataWithIndicators.length} -> ${finalChartData.length}`);
        } else {
          console.log(`[Analyze] 最后一个数据点已经是16:00，无需添加占位点`);
        }
      }
      ...
      <ResponsiveContainer width="100%" height={chartHeight}>
        <LineChart data={chartData}>
          ...
          <XAxis
            ...
            ticks={
              selectedTimeframe === '1D' ? getProfessional1DayTicks(chartData) :
  ...
  const renderRSIChart = () => {
    // 检查是否有有效的RSI数据
    const hasValidRSIData = chartData.some(d => d.rsi !== undefined && !isNaN(d.rsi) && d.rsi > 0);
    
    if (chartData.length === 0 || !hasValidRSIData) {
      return (
        <Empty
    ...
    console.log(`[RSI专业优化] 时间范围: ${selectedTimeframe}, 有效RSI数据点: ${rsiData.length}/${chartData.length}`);
    if (rsiData.length > 0) {
      const rsiValues = rsiData.map(d => d.rsi!).filter(v => !isNaN(v));
      if (rsiValues.length > 0) {
        console.log(`[RSI专业优化] RSI范围: ${Math.min(...rsiValues).toFixed(1)} - ${Math.max(...rsiValues).toFixed(1)}`);
      }
    }
            ...
            <div style={{ padding: '10px' }}>
              ...
              <div style={{
                ...
                {(() => {
                  ...
                  if (!lastData || lastData.volume === undefined) return '--';
          ...
          >
            <div style={{ padding: '10px' }}>
              ...
              <div style={{
                ...
                {(() => {
                  if (chartData.length === 0 || !stockData.price) return '--';
                  ...
                  if (fiftyTwoWeekHigh === fiftyTwoWeekLow) return '--';
              ...
              <div style={{
                ...
                {(() => {
                  if (chartData.length === 0 || !stockData.price) return 'No data';
                  ...
                  if (fiftyTwoWeekHigh === fiftyTwoWeekLow) return 'No range';
                  
                  const rangePosition = ((currentPrice - fiftyTwoWeekLow) / (fiftyTwoWeekHigh - fiftyTwoWeekLow)) * 100;
                  return `${safeToFixed(rangePosition, 1)}% of range`;
                })()}
              </div>
            </div>
          </Card>
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    {(() => {
                      ...
                      if (sma20 !== undefined && sma50 !== undefined) {
                        return 'Macro trend direction based on moving average alignment';
                      } else {
                        return 'Provisional macro trend assessment';
                      }
                    })()}
                  </div>
                </div>
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    {(() => {
                      ...
                      if (rsi === undefined || isNaN(rsi)) return 'N/A';
                  ...
                  <div style={{
                    ...
                    {(() => {
                      ...
                      if (rsi === undefined || isNaN(rsi)) return 'Not calculated';
                      
                      return `RSI: ${safeToFixed(rsi, 1)}`;
                    })()}
                  </div>
                </div>
              </Col>
              ...
              <Col xs={24} sm={12} md={8}>
                <div style={{
                  ...
                  <div style={{
                    ...
                    {(() => {
                      if (chartData.length === 0 || !stockData.price) return 'N/A';
                  ...
                  <div style={{
                    ...
                    {(() => {
                      ...
                      if (fiftyTwoWeekHigh === fiftyTwoWeekLow) return 'Flat range';
                      
                      const rangePosition = ((currentPrice - fiftyTwoWeekLow) / (fiftyTwoWeekHigh - fiftyTwoWeekLow)) * 100;
                      return `${safeToFixed(rangePosition, 1)}% of 52W range`;
                    })()}
                  </div>
                </div>
              </Col>
              ...
              <Col xs={24} sm={12} md={6}>
                <div style={{
                  ...
                  <div style={{
                    ...
                    {(() => {
                      if (!stockData.price || chartData.length === 0) return '--';
                      
                      const lastData = chartData[chartData.length - 1];
                      const price = stockData.price;
                      const sma20 = lastData?.sma20;
                      
                      if (sma20 === undefined || isNaN(sma20)) return 'No SMA20';
                      
                      const diff = price - sma20;
                  ...
                  <div style={{
                    ...
                    {(() => {
                      ...
                      if (sma20 === undefined || isNaN(sma20)) return 'SMA20: N/A';
                      
                      return `Price: $${safeToFixed(price, 2)} | SMA20: $${safeToFixed(sma20, 2)}`;
                    })()}
                  </div>
                </div>
              </Col>
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    {(() => {
                      ...
                      if (sma50 === undefined || isNaN(sma50)) return 'Need 50 periods';
                      
                      const diff = price - sma50;
                  ...
                  <div style={{
                    ...
                    {(() => {
                      ...
                      if (sma50 === undefined || isNaN(sma50)) return 'SMA50: N/A';
                      
                      return `Price: $${safeToFixed(price, 2)} | SMA50: $${safeToFixed(sma50, 2)}`;
                    })()}
                  </div>
                </div>
              </Col>
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    {(() => {
                      ...
                      return `Diff: ${safeToFixed(diffPercent, 2)}%`;
                    })()}
                  </div>
                </div>
              </Col>
              ...
              <Col xs={24} sm={12} md={6}>
                <div style={{
                  ...
                  <div style={{ marginBottom: '16px' }}>
                    ...
                    <div style={{
                      ...
                      {(() => {
                        ...
                        const support1 = Math.min(...recentLows) * 0.98; // 近期低点下方2%
                        const support2 = Math.min(...recentLows) * 0.95; // 近期低点下方5%
                  ...
                  <div>
                    ...
                    <div style={{
                      ...
                      {(() => {
                        ...
                        const resistance1 = Math.max(...recentHighs) * 1.02; // 近期高点上方2%
                        const resistance2 = Math.max(...recentHighs) * 1.05; // 近期高点上方5%
                  ...
                  <div style={{
                    ...
                    Derived from recent swing highs/lows in {selectedTimeframe} timeframe
                  </div>
                </div>
              </Col>
