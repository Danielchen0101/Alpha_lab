...
// Attach Supabase access token so backend can resolve per-user config, refreshing if needed
...
export const TIMEFRAMES: Record<string, { interval: string; range: string; dataPoints: number; label: string }> = {
  '1D': { interval: '5min', range: '1day', dataPoints: 78, label: '1 Day' },    // 5分钟粒度，78个数据点（6.5小时×12个5分钟段）
  '1W': { interval: '1h', range: '1week', dataPoints: 40, label: '1 Week' },    // 1小时粒度，40个数据点（5天×8个交易小时）
  '1M': { interval: '1day', range: '1month', dataPoints: 22, label: '1 Month' }, // 日线粒度，22个交易日
  '3M': { interval: '1day', range: '3month', dataPoints: 66, label: '3 Months' }, // 日线粒度，66个交易日
  '1Y': { interval: '1day', range: '1year', dataPoints: 252, label: '1 Year' },   // 日线粒度，252个交易日
};
...
export const getStockData = async (symbol: string): Promise<StockData> => {
  try {
    ...
    if (response.data && !response.data.error) {
      // 后端已经返回正确的字段名：dayHigh, dayLow, previousClose, yearHigh, yearLow
