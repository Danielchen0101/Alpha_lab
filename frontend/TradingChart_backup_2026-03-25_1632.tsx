...
const TradingChart: React.FC<TradingChartProps> = ({ data, height = 500, parameters }) => {
  ...
  // Calculate chart heights with explicit pixel values - 使用明确像素值，确保图表足够高
  const priceChartHeight = (hasVolumeData && showVolume) ? 380 : 480; // Price Chart明确高度：380px（有Volume时）或480px（无Volume时）
  const volumeChartHeight = (hasVolumeData && showVolume) ? 180 : 0; // Volume Chart明确高度：180px，确保不是薄带
  ...
  return (
    <div style={{ 
      ...
      <div style={{ 
        ...
        marginBottom: (hasVolumeData && showVolume) ? '28px' : '0',  // 明确间距：28px，让两个图之间有适当分隔
