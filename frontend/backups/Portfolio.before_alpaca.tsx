...
import { Card, Row, Col, Statistic, Table, Typography, Progress, Tag, Button, message, Popconfirm, Input, InputNumber, Select, Radio, Form, Divider, Space, Alert } from 'antd';
...
const DESIGN_SYSTEM = {
  ...
  colors: {
    ...
    success: '#52c41a',      // 成功/盈利 - 绿色
    warning: '#faad14',      // 警告 - 橙色
    error: '#ff4d4f',        // 错误/亏损 - 红色
    info: '#1890ff',         // 信息 - 蓝色
    text: {
...
const styles = {
  ...
  sectionTitle: {
    fontSize: DESIGN_SYSTEM.typography.h3.size,
    fontWeight: DESIGN_SYSTEM.typography.h3.weight,
    marginBottom: DESIGN_SYSTEM.spacing.sm,
    color: DESIGN_SYSTEM.colors.text.primary,
  },
  subsectionTitle: {
    fontSize: DESIGN_SYSTEM.typography.h4.size,
    fontWeight: DESIGN_SYSTEM.typography.h4.weight,
  ...
  description: {
    fontSize: DESIGN_SYSTEM.typography.caption.size,
...
// 导入Broker Service
import { brokerService, setBrokerMode, getBrokerMode } from '../services/brokerService';
...
const Portfolio: React.FC = () => {
  ...
  const saveSessionToHistory = useCallback((sessionSnapshot: typeof paperTradingSession, endEquity: number, endTime: string) => {
    ...
    setSessionHistory(prev => {
      ...
      // 添加 debug 日志
      console.log('Saved session:', historyItem);
  ...
  const executeTrade = async (order: TradeOrder, source: 'DEMO_BUTTON' | 'MANUAL_ORDER' | 'STRATEGY_SIGNAL' = 'MANUAL_ORDER'): Promise<boolean> => {
    // 根据 broker mode 选择执行路径
    if (getBrokerMode() === 'ALPACA_PAPER') {
      // ALPACA_PAPER mode: 调用 broker service 下单
      try {
        console.log(`[Alpaca Paper] 执行交易: ${order.action} ${order.qty || order.shares} ${order.symbol}`);
        
        // 构建 broker order
        const brokerOrder: TradeOrder = {
          symbol: order.symbol,
          qty: order.qty || order.shares || 1,
          side: order.action.toLowerCase() as 'buy' | 'sell',
          type: 'market',
          time_in_force: 'day'
        };
        
        // 调用 broker service 下单
        const placedOrder = await brokerService.placeOrder(brokerOrder);
        
        // 下单成功后刷新数据
        message.success(`订单提交成功: ${placedOrder.symbol} ${placedOrder.side} ${placedOrder.qty} (ID: ${placedOrder.id})`);
        
        // 刷新账户、持仓、订单数据
        try {
          // 获取最新账户信息
          const account = await brokerService.getAccount();
          console.log('[Alpaca Paper] 账户更新:', account);
          
          // 获取最新持仓
          const positions = await brokerService.getPositions();
          console.log('[Alpaca Paper] 持仓更新:', positions);
          
          // 获取最新订单
          const orders = await brokerService.getOrders();
          console.log('[Alpaca Paper] 订单更新:', orders);
          
          // 这里可以更新页面状态，但为了简化，先只显示成功消息
          // 后续可以添加状态更新逻辑
          
        } catch (refreshError) {
          console.error('[Alpaca Paper] 刷新数据失败:', refreshError);
          // 即使刷新失败，订单已经成功提交
        }
        ...
        console.error('[Alpaca Paper] 下单失败:', error);
        message.error(`下单失败: ${error.message || '未知错误'}`);
        return false;
      }
    } else {
      // LOCAL mode: 继续使用本地模拟交易
  ...
  return (
    <div style={{ 
      ...
      backgroundColor: DESIGN_SYSTEM.colors.background.page,
      minHeight: '100vh'
    }}>
      ...
      <div style={{ marginBottom: DESIGN_SYSTEM.spacing.lg }}>
        <div style={{ 
          ...
          <h1 style={styles.sectionTitle}>Portfolio Dashboard</h1>
        </div>
        <Text style={styles.description}>
          Overview of your investment portfolio, positions, and trading performance
        </Text>
      </div>

      {/* ========== 核心指标卡片 ========== */}
      <div style={{ marginBottom: DESIGN_SYSTEM.spacing.lg }}>
        <h2 style={styles.subsectionTitle}>Portfolio Summary</h2>
        <Row gutter={[DESIGN_SYSTEM.spacing.sm, DESIGN_SYSTEM.spacing.sm]}>
          <Col xs={24} sm={12} lg={6}>
            <Card style={styles.card}>
              ...
              <div style={styles.statValue(DESIGN_SYSTEM.colors.text.primary)}>
                ${totalEquity.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </div>
              <Text style={styles.description}>Total account value</Text>
            </Card>
          </Col>
          <Col xs={24} sm={12} lg={6}>
            <Card style={styles.card}>
              ...
              <Text style={styles.description}>Available for trading</Text>
            </Card>
          </Col>
          <Col xs={24} sm={12} lg={6}>
            <Card style={styles.card}>
              ...
              <Text style={styles.description}>Current positions value</Text>
            </Card>
          </Col>
          <Col xs={24} sm={12} lg={6}>
            <Card style={styles.card}>
              <div style={styles.statTitle}>
                {totalPnL >= 0 ? (
                  <ArrowUpOutlined style={{ marginRight: '6px', color: DESIGN_SYSTEM.colors.success }} />
                ) : (
                  <ArrowDownOutlined style={{ marginRight: '6px', color: DESIGN_SYSTEM.colors.error }} />
                )}
                Total P&L
              </div>
              <div style={styles.statValue(totalPnL >= 0 ? DESIGN_SYSTEM.colors.success : DESIGN_SYSTEM.colors.error)}>
                ${totalPnL.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                <span style={{ 
                  fontSize: '14px', 
                  marginLeft: '6px',
                  color: totalPnL >= 0 ? DESIGN_SYSTEM.colors.success : DESIGN_SYSTEM.colors.error
                }}>
                  ({totalPnL >= 0 ? '+' : ''}{pnlPercentage.toFixed(2)}%)
                </span>
              </div>
              <Text style={styles.description}>Total profit & loss</Text>
            </Card>
          </Col>
        </Row>
      </div>
      ...
      {equityHistory.length > 0 && (() => {
        ...
        return (
          <Card style={{ ...styles.card, marginBottom: DESIGN_SYSTEM.spacing.lg }}>
            <div style={{ marginBottom: DESIGN_SYSTEM.spacing.sm }}>
              <h3 style={styles.cardTitle}>Equity Curve</h3>
              <Text style={styles.description}>
            ...
            {/* 统计信息卡片 */}
            <Row gutter={[DESIGN_SYSTEM.spacing.sm, DESIGN_SYSTEM.spacing.sm]} style={{ marginBottom: DESIGN_SYSTEM.spacing.md }}>
              <Col xs={24} sm={8}>
                <Card style={{ 
                  ...styles.card, 
                  border: 'none',
                  ...
                  <div style={styles.statTitle}>Current Equity</div>
                  <div style={styles.statValue(DESIGN_SYSTEM.colors.text.primary)}>
              ...
              <Col xs={24} sm={8}>
                <Card style={{ 
                  ...
                  <div style={styles.statTitle}>High</div>
                  <div style={styles.statValue(DESIGN_SYSTEM.colors.success)}>
                    ${highEquity.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </div>
                </Card>
              </Col>
              <Col xs={24} sm={8}>
                <Card style={{ 
                  ...
                  <div style={styles.statTitle}>Low</div>
                  <div style={styles.statValue(DESIGN_SYSTEM.colors.error)}>
                    ${lowEquity.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </div>
                </Card>
              </Col>
            </Row>
                ...
                if (paperTradingSession.totalTrades > 0 && paperTradingSession.startTime) {
                  ...
                  setSessionHistory(prev => {
                    ...
                    try {
                      ...
                      // 添加 debug 日志
                      console.log('Saved session (Stop button):', historyItem);
      ...
      <Card 
        ...
        {/* ========== Session History ========== */}
        {sessionHistory.length > 0 && (
          <Card style={{ ...styles.card, marginTop: DESIGN_SYSTEM.spacing.lg }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: DESIGN_SYSTEM.spacing.sm }}>
              <h3 style={styles.subsectionTitle}>Paper Trading Session History</h3>
              <Button 
                type="link" 
                size="small"
                onClick={() => {
      ...
      {/* Broker Mode Test Panel */}
      <Card 
        title="Broker Mode Test" 
        style={{ marginTop: '24px', backgroundColor: '#fff7e6' }}
        extra={
          <Tag color={getBrokerMode() === 'LOCAL' ? 'blue' : 'green'}>
            {getBrokerMode() === 'LOCAL' ? 'LOCAL MODE' : 'ALPACA PAPER MODE'}
          </Tag>
        }
      >
        <Row gutter={[16, 16]}>
          <Col span={24}>
            <Text type="secondary">
              Test broker service integration. Current mode: <strong>{getBrokerMode()}</strong>
            </Text>
          </Col>
          <Col span={24}>
            <Space>
              <Button 
                type={getBrokerMode() === 'LOCAL' ? 'primary' : 'default'}
                onClick={() => {
                  setBrokerMode('LOCAL');
                  message.success('Switched to LOCAL mode');
                }}
              >
                Local Mode
              </Button>
              <Button 
                type={getBrokerMode() === 'ALPACA_PAPER' ? 'primary' : 'default'}
                onClick={() => {
                  setBrokerMode('ALPACA_PAPER');
                  message.success('Switched to ALPACA PAPER mode');
                }}
              >
                Alpaca Paper Mode
              </Button>
          ...
          <Col span={24}>
            <Divider orientation="left">Test Broker API</Divider>
            <Space>
              <Button 
                size="small"
                onClick={async () => {
                  try {
                    const account = await brokerService.getAccount();
                    message.success(`Account loaded: ${account.account_number}`);
                    console.log('Account info:', account);
                  } catch (error: any) {
                    message.error(`Failed to load account: ${error.message || 'Unknown error'}`);
                  }
                }}
              >
                Test Get Account
              ...
              <Button 
                ...
                onClick={async () => {
                  try {
                    const positions = await brokerService.getPositions();
                    message.success(`Positions loaded: ${positions.length} positions`);
                    console.log('Positions:', positions);
                  } catch (error: any) {
                    message.error(`Failed to load positions: ${error.message || 'Unknown error'}`);
                  }
                }}
              >
                Test Get Positions
              ...
              <Button 
                ...
                onClick={async () => {
                  try {
                    const orders = await brokerService.getOrders();
                    message.success(`Orders loaded: ${orders.length} orders`);
          ...
          <Col span={24}>
            <Alert 
              ...
              description={
                <div>
                  <div>• LOCAL mode uses simulated data</div>
                  <div>• ALPACA PAPER mode requires valid API credentials in .env.local</div>
                  <div>• Only read operations are implemented in this step</div>
                </div>
              }
              showIcon
            />
