...
const TradingChart: React.FC<TradingChartProps> = ({ data, height = 500, parameters }) => {
  ...
  // Calculate chart heights with optimized proportions - 显著增加高度，让Trading Chart整体明显扩大
  const priceChartHeight = (hasVolumeData && showVolume) ? height * 0.80 : height * 0.90; // Price Chart显著增加高度，更接近Equity Curve
  const volumeChartHeight = (hasVolumeData && showVolume) ? height * 0.20 : 0; // Volume Chart也增加高度，但保持主次分明
  ...
  return (
    <div style={{ 
      ...
      <div style={{ 
        ...
        marginBottom: (hasVolumeData && showVolume) ? '45px' : '0'  // 增加间距，让两个图之间更舒展
      }}>
      ...
      {hasVolumeData && showVolume && (
        <div style={{ 
          ...
          marginTop: '10px',  // 减少顶部间距，让两个图更紧凑
          marginBottom: '30px' // 增加底部间距，让整体更舒展
        }}>
          <div style={{ 
            display: 'flex', 
            justifyContent: 'space-between', 
