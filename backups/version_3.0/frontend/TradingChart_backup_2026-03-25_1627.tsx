...
const TradingChart: React.FC<TradingChartProps> = ({ data, height = 500, parameters }) => {
  ...
  // Calculate chart heights with optimized proportions - 在外层容器变高后，重新分配内部图表高度
  const priceChartHeight = (hasVolumeData && showVolume) ? height * 0.75 : height * 0.85; // Price Chart保持较高比例，但给Volume更多空间
  const volumeChartHeight = (hasVolumeData && showVolume) ? height * 0.25 : 0; // Volume Chart显著增加高度，充分利用外层容器空间
  ...
  return (
    <div style={{ 
      border: '1px solid #e8e8e8', 
      borderRadius: '8px', 
      padding: '24px', // 增加内边距，让整体更舒展
      backgroundColor: '#fff',
      minHeight: height + 80, // 显著增加外层容器高度，给内部图表更多空间
      display: 'flex',
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          <Space direction="horizontal" size={8}>
            ...
            >
              <span style={{ color: '#1890ff', fontWeight: '700', letterSpacing: '0.3px' }}>PRICE</span>
            </Checkbox>
      ...
      <div style={{ 
        ...
        marginBottom: (hasVolumeData && showVolume) ? '35px' : '0',  // 调整间距，让布局更平衡
        flex: '1 0 auto'  // 允许Price Chart在外层容器中灵活调整
      }}>
      ...
      {hasVolumeData && showVolume && (
        <div style={{ 
          ...
          marginTop: '15px',  // 增加顶部间距，让两个图之间有适当分隔
          marginBottom: '20px', // 减少底部间距，让整体更紧凑
          flex: '1 0 auto'  // 允许Volume Chart在外层容器中灵活调整
        }}>
          <div style={{ 
            display: 'flex', 
            justifyContent: 'space-between', 
