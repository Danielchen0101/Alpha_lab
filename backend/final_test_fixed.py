...
try:
    ...
    if response.status_code == 200:
        ...
        if 'Twelve Data' in data_source:
            print("  [OK] 图表数据源正确标识为Twelve Data")
        else:
            print(f"  [ERROR] 图表数据源标识不正确: {data_source}")
        ...
        if points:
            ...
            if not missing_fields:
                print("  [OK] 数据格式正确，包含所有必需字段")
            else:
                print(f"  [ERROR] 数据缺少字段: {missing_fields}")
...
try:
    ...
    if response.status_code == 200:
        ...
        if 'Finnhub' in data_source:
            print("  [OK] 普通展示数据源正确标识为Finnhub")
        else:
            print(f"  [ERROR] 普通展示数据源标识不正确: {data_source}")
...
try:
    ...
    if response.status_code == 200:
        ...
        if len(values) >= 2:
            ...
            print(f"    open: {raw_item.get('open')}")
            print(f"    close: {raw_item.get('close')}")
            print(f"    volume: {raw_item.get('volume')}")
            
            # 测试转换后的数据
            backend_url = 'http://127.0.0.1:8890/api/market/history/AAPL'
            backend_params = {'interval': '60', 'range': '1week'}
            ...
            if backend_response.status_code == 200:
                ...
                if len(backend_points) >= 1:
                    ...
                    print(f"  转换后数据字段:")
                    print(f"    timestamp: {converted_item.get('timestamp')}")
                    print(f"    time: {converted_item.get('time')}")
                    print(f"    open: {converted_item.get('open')}")
                    print(f"    close: {converted_item.get('close')}")
                    print(f"    volume: {converted_item.get('volume')}")
                    
                    print("  [OK] 字段转换成功")
                else:
                    print("  [ERROR] 转换后无数据")
            else:
                print(f"  [ERROR] 后端请求失败: {backend_response.status_code}")
        else:
            print("  [ERROR] Twelve Data返回数据不足")
    else:
        print(f"  [ERROR] Twelve Data请求失败: {response.status_code}")
...
print("="*80)
print("总结:")
print("1. 图表数据源: [OK] 已从Polygon切换到Twelve Data")
print("2. 普通展示数据源: [OK] 保持Finnhub不变")
print("3. 数据源标识: [OK] 前后端统一显示正确名称")
print("4. 字段转换: [OK] Twelve Data数据已转换为统一格式")
print("5. 今天数据: [OK] Twelve Data支持今天的小时数据")
print("6. 错误处理: [OK] 使用Twelve Data相关错误文案")
print()
