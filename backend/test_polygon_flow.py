...
print("\n1. 测试健康检查:")
try:
    response = requests.get("http://127.0.0.1:8890/api/health", timeout=5)
...
print("\n2. 测试单个股票 (AAPL):")
try:
    response = requests.get("http://127.0.0.1:8890/api/market/stock/AAPL", timeout=10)
    print(f"   状态: {response.status_code}")
...
print("\n4. 测试股票列表:")
try:
    response = requests.get("http://127.0.0.1:8890/api/market/stocks", 
