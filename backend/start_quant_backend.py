...
alpaca_config_state = {
    'paper_api_key': ALPACA_API_KEY,  # 直接使用从config.py导入的变量
    'paper_api_secret': ALPACA_API_SECRET,  # 直接使用从config.py导入的变量
...
# 打印 Alpaca 配置状态（安全掩码）
key_preview = f"{alpaca_config_state['paper_api_key'][:6]}...{alpaca_config_state['paper_api_key'][-4:]}" if alpaca_config_state['paper_api_key'] else "None"
secret_len = len(alpaca_config_state['paper_api_secret']) if alpaca_config_state['paper_api_secret'] else 0
print(f"[Alpaca配置] 环境: {alpaca_config_state['environment']}")
print(f"[Alpaca配置] Paper API Key (掩码): {key_preview}")
print(f"[Alpaca配置] Paper API Secret 长度: {secret_len} 字符")
...
def fetch_alpaca_bars(symbol, timeframe, range_param):
    ...
    try:
        ...
        if environment == 'paper':
            ALPACA_API_KEY = alpaca_config_state.get('paper_api_key')
            ALPACA_SECRET_KEY = alpaca_config_state.get('paper_api_secret')
        else:
