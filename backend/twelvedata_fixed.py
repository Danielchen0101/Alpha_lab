...
def get_stock_detail(symbol):
    ...
    if stock_data.get('success', False):
        ...
        # 移除success字段
        stock_data.pop('success', None)
        return jsonify(stock_data), 200
    else:
        print(f"[Finnhub] 获取股票详情失败")
        # 移除success字段
        stock_data.pop('success', None)
        return jsonify(stock_data), 200
...
def get_market_stocks():
    ...
    for symbol in symbols:
        ...
        if stock_data.get('success', False):
            ...
            stock_data.pop('success', None)
            stocks.append(stock_data)
            print(f"[Finnhub] ✗ 获取 {symbol} 数据失败")
    
    print(f"[Finnhub] 成功获取 {success_count}/{len(symbols)} 支股票数据")
...
if __name__ == '__main__':
    print("=" * 80)
    print("Twelve Data图表数据源后端启动 (修复版)")
