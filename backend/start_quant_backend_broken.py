...
def fetch_alpaca_bars(symbol, timeframe, range_param):
    ...
    try:
        ...
        if range_param == '1D':
            # 1D: 今天00:00 AM 美东时间 到 当前美东时间往前15分钟
            # 今天00:00 AM 美东时间
            today_start_eastern = now_eastern.replace(hour=0, minute=0, second=0, microsecond=0)
            
            # 当前美东时间往前15分钟（15分钟延迟数据）
            end_eastern = now_eastern - datetime.timedelta(minutes=15)
            
            # 确保结束时间不早于开始时间
            if end_eastern < today_start_eastern:
                print(f'[Alpaca bars] 警告: 结束时间{end_eastern.strftime("%H:%M:%S")}早于开始时间{today_start_eastern.strftime("%H:%M:%S")}')
                print(f'[Alpaca bars] 使用开始时间作为结束时间')
                end_eastern = today_start_eastern + datetime.timedelta(minutes=5)  # 至少5分钟间隔
            
            # 转换为UTC
            today_start_utc = today_start_eastern.astimezone(utc)
            ...
            print(f'  - 开始: {today_start_eastern.strftime("%Y-%m-%d %H:%M:%S")} EDT')
            print(f'  - 结束: {end_eastern.strftime("%Y-%m-%d %H:%M:%S")} EDT (当前时间{now_eastern.strftime("%H:%M:%S")}往前15分钟)')
        ...
        # 如果sip失败，尝试iex
        if response.status_code != 200:
            print(f'[Alpaca bars] feed=sip请求失败: {response.status_code}, 错误: {response.text[:200]}')
...
def fetch_alpaca_bars_for_backtest(symbol, timeframe, start_date_utc, end_date_utc, limit=1000):
    ...
    try:
        ...
        print(f'[Alpaca Backtest Bars] 请求参数: {params}')
        ...
        if response.status_code == 200:
            ...
            if 'bars' in data and data['bars']:
                bars = data['bars']
                print(f'[Alpaca Backtest Bars] 成功获取 {len(bars)} 条bars数据')
                ...
                print(f'[Alpaca Backtest Bars] 响应中没有bars数据: {data}')
                return [], False, 'Alpaca响应中没有bars数据'
        else:
            print(f'[Alpaca Backtest Bars] API请求失败: {response.status_code} - {response.text}')
            return [], False, f'Alpaca API请求失败: {response.status_code}'
...
def get_stock_detail(symbol):
    ...
    try:
        ...
        stock_info = {
            ...
            "dataSource": "Alpaca" if alpaca_data else "Finnhub",
            "timestamp": int(time.time()),
        ...
        # 如果Alpaca数据缺失，使用Finnhub quote数据作为备选
        elif quote_data:
            print(f'[股票详情] 使用Finnhub quote数据作为备选')
            stock_info.update({
...
def run_parameter_optimization():
    ...
    try:
        ...
        if days_diff <= 30:
            timeframe = '1Day'  # 1个月以内用日线
        elif days_diff <= 90:
            timeframe = '1Day'  # 3个月以内用日线
        else:
            timeframe = '1Day'  # 超过3个月也用日线
        
        print(f"[Optimization] 时间范围: {days_diff} 天，使用timeframe: {timeframe}")
        
        # 获取Alpaca历史数据
        historical_data = get_alpaca_history_for_backtest(symbol, timeframe, f"{days_diff}day")
        
        if not historical_data or len(historical_data) == 0:
            print(f"[Optimization] 错误: 无法获取Alpaca历史数据")
            return jsonify({
                "success": False,
                "error": f"无法获取Alpaca历史数据: {symbol}",
        ...
        print(f"[Optimization] 获取到 {len(historical_data)} 条Alpaca历史数据")
