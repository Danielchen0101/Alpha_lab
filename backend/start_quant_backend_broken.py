    ...
    try:
        if os.path.exists(AI_CONFIG_FILE):
            ...
            if 'provider' in saved_config:
                ai_provider_config_state['provider'] = saved_config['provider']
            if 'apiKey' in saved_config:
                ai_provider_config_state['apiKey'] = saved_config['apiKey']
            if 'baseURL' in saved_config:
                ai_provider_config_state['baseURL'] = saved_config['baseURL']
            if 'baseUrl' in saved_config:
                ai_provider_config_state['baseURL'] = saved_config['baseUrl']  # 统一使用baseURL
            if 'model' in saved_config:
                ai_provider_config_state['model'] = saved_config['model']
...
def get_stock_news(symbol):
    """获取股票新闻接口 - 先尝试Alpaca，再尝试Finnhub，最后使用模拟数据"""
    ...
    try:
        ...
        # 3. 如果都没有真实新闻，添加模拟新闻数据
        if not news_items:
            print(f'[新闻接口] 没有找到真实新闻，添加模拟新闻数据')
            
            # 根据股票符号创建不同的模拟新闻
            mock_news_templates = {
                'AAPL': {
                    'headline': 'Apple Reports Strong Quarterly Earnings, Beats Expectations',
                    'source': 'Financial Times',
                    'summary': 'Apple Inc. reported better-than-expected quarterly earnings, driven by strong iPhone sales and growing services revenue. The company also announced a new share buyback program.',
                    'sentiment': 'Positive'
                },
                'META': {
                    'headline': 'Meta Platforms Announces Major AI Investment and Restructuring',
                    'source': 'Wall Street Journal',
                    'summary': 'Meta Platforms unveiled a significant investment in artificial intelligence infrastructure and announced a corporate restructuring to focus on AI and metaverse development.',
                    'sentiment': 'Mixed'
                },
                'MSFT': {
                    'headline': 'Microsoft Azure Cloud Revenue Growth Accelerates',
                    'source': 'Bloomberg',
                    'summary': 'Microsoft reported accelerated growth in its Azure cloud computing business, exceeding analyst expectations and gaining market share against competitors.',
                    'sentiment': 'Positive'
                },
                'GOOGL': {
                    'headline': 'Google Parent Alphabet Announces AI Breakthrough and Cost Cuts',
                    'source': 'Reuters',
                    'summary': 'Alphabet announced significant advancements in its AI research while implementing cost-cutting measures across several divisions to improve profitability.',
                    'sentiment': 'Neutral'
                },
                'AMZN': {
                    'headline': 'Amazon Web Services Secures Major Government Contract',
                    'source': 'CNBC',
                    'summary': 'Amazon Web Services won a significant government cloud computing contract, boosting its position in the competitive public sector market.',
                    'sentiment': 'Positive'
                }
            }
            
            # 获取股票对应的模拟新闻或使用默认模板
            template = mock_news_templates.get(symbol_upper, {
                'headline': f'{symbol_upper} Reports Quarterly Results',
                'source': 'Financial News',
                'summary': f'{symbol_upper} reported quarterly financial results. Analysts are reviewing the earnings report and providing updated price targets.',
                'sentiment': 'Neutral'
            })
            ...
            mock_news = {
                ...
                'datetime': '2026-04-15T01:00:18.399851Z',
                'summary': template['summary'],
                'url': f'https://example.com/news/{symbol_upper.lower()}-earnings',
                'sentiment': template['sentiment']
            }
            
            news_items = [mock_news]
            source = 'mock'
            print(f'[新闻接口] 添加模拟新闻: {mock_news["headline"]}')
        ...
        if news_items and len(news_items) > 0:
            ...
            # 格式化top_news
            formatted_top_news = {
                'headline': top_news.get('headline') or top_news.get('title') or 'No title',
                'source': top_news.get('source') or (source.capitalize() if source else 'Unknown'),
                'datetime': top_news.get('published_at') or top_news.get('datetime') or top_news.get('time'),
                'summary': top_news.get('summary') or top_news.get('content', '')[:200] + '...',
                'url': top_news.get('url') or top_news.get('link'),
                'provider': source or 'mock'
            }
            
            # 分析新闻情绪
            sentiment = 'Neutral'
            if top_news.get('sentiment'):
                sentiment = top_news.get('sentiment')
            elif 'positive' in str(top_news.get('summary', '')).lower() or 'beat' in str(top_news.get('summary', '')).lower():
                sentiment = 'Positive'
            elif 'negative' in str(top_news.get('summary', '')).lower() or 'miss' in str(top_news.get('summary', '')).lower():
                sentiment = 'Negative'
            
            # 分析事件风险
            event_risk = 'Low'
            risk_keywords = ['crisis', 'lawsuit', 'investigation', 'warning', 'decline', 'drop', 'plunge']
            summary_lower = str(top_news.get('summary', '')).lower()
            if any(keyword in summary_lower for keyword in risk_keywords):
                event_risk = 'Medium'
            if 'bankruptcy' in summary_lower or 'fraud' in summary_lower or 'collapse' in summary_lower:
                event_risk = 'High'
            ...
            print(f'[新闻接口] 返回新闻数据: {len(news_items)}条新闻，情绪: {sentiment}，风险: {event_risk}')
            return jsonify(response_data)
        
        # 如果执行到这里，说明news_items为空，即使添加了模拟新闻
        # 作为安全措施，创建一个默认的模拟新闻
        print(f'[新闻接口] 安全措施：创建默认模拟新闻')
        
        default_mock_news = {
            'headline': f'{symbol_upper} Market Update',
            'source': 'Financial News',
            'datetime': '2026-04-15T01:00:18.399851Z',
            'summary': f'{symbol_upper} is trading in the market with normal activity.',
            'url': f'https://example.com/news/{symbol_upper.lower()}-update',
            'sentiment': 'Neutral'
        }
        
        news_items = [default_mock_news]
        source = 'mock'
        
        # 现在news_items不为空，可以继续执行新闻分析逻辑
        # 重新执行新闻分析逻辑
        if news_items and len(news_items) > 0:
            # 按时间排序，选择最新的
            sorted_news = sorted(news_items, 
            ...
            print(f'[新闻接口] 返回新闻数据(安全措施): {len(news_items)}条新闻，情绪: {sentiment}，风险: {event_risk}')
            return jsonify(response_data)
        
        # 如果连安全措施都失败了，返回错误
        print(f'[新闻接口] 严重错误：无法创建新闻数据')
        return jsonify({
            'success': False,
            'error': '无法创建新闻数据',
            'symbol': symbol_upper,
