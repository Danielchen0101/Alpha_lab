...
def fetch_alpaca_stock_data(symbol):
    ...
    try:
        ...
        if daily_bars_response.status_code == 200:
            ...
            if len(daily_bars) > 0:
                # 最新日线bar的收盘价作为previousClose
                latest_daily_bar = daily_bars[0]
                previous_close = float(latest_daily_bar.get('c')) if latest_daily_bar.get('c') else None
                daily_bars_data = latest_daily_bar
                print(f'[Alpaca数据] 股票 {symbol} 获取到日线bars数据，previousClose: {previous_close}')
            else:
...
def fetch_alpaca_stock_data_snapshot(symbols):
    ...
    try:
        ...
        for symbol in symbols:
            ...
            elif daily_bar.get('t'):
                timestamp = daily_bar.get('t')

            # 4. OHLCV数据
            open_price = float(daily_bar.get('o')) if daily_bar.get('o') else None
            day_high = float(daily_bar.get('h')) if daily_bar.get('h') else None
            day_low = float(daily_bar.get('l')) if daily_bar.get('l') else None
            volume = int(daily_bar.get('v')) if daily_bar.get('v') else None

            # 5. previousClose
            previous_close = float(prev_daily_bar.get('c')) if prev_daily_bar.get('c') else None
...
def fetch_alpaca_bars(symbol, timeframe, range_param):
    ...
    try:
        ...
        if environment == 'paper':
            ALPACA_API_KEY = alpaca_config_state.get('paper_api_key', 'PKFQZZXERLVJLJHODHPPEB52RD')
            ALPACA_SECRET_KEY = alpaca_config_state.get('paper_api_secret', '5odo2jBF7YFLa7DAvss3hV7WVXE789ktTor7zMyPewxa')
        else:
            ALPACA_API_KEY = alpaca_config_state.get('live_api_key', '')
            ALPACA_SECRET_KEY = alpaca_config_state.get('live_api_secret', '')
        ...
        print(f'[Alpaca bars] 环境: {environment}, API Key前5位: {ALPACA_API_KEY[:5]}...')
...
def ai_market_scanner():
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            ...
            future_to_symbol = {}
            for symbol in symbols:
                future = executor.submit(get_stock_data_for_scanner, symbol)
            ...
            for future in as_completed(future_to_symbol):
                ...
                try:
                    stock_data, news_data, profile_data, analysis_result = future.result()
                    ...
                    if stock_data and 'price' in stock_data and stock_data['price']:
                        ...
                        print(f'扫描完成: {symbol} - {analysis_result.get("trendLabel", "Neutral")}')
                    else:
    ...
    try:
        ...
        try:
            ...
            if alpaca_data and not alpaca_error and alpaca_data.get('price'):
                # 调试：查看Alpaca返回的volume数据
                alpaca_volume = alpaca_data.get('volume')
                print(f'[DEBUG] {symbol} Alpaca volume: {alpaca_volume}, type: {type(alpaca_volume)}')
                if alpaca_volume is None or alpaca_volume == 0:
                    print(f'[DEBUG] {symbol} Alpaca volume is None or 0, checking bars data')
                    # 尝试从原始bars数据获取volume
                    if 'bars_data' in alpaca_data:
                        print(f'[DEBUG] {symbol} bars_data available: {alpaca_data.get("bars_data")}')
                
                # 确保volume有值，如果Alpaca没有则尝试Finnhub
                final_volume = alpaca_volume if alpaca_volume is not None else 0
                data_source = 'Alpaca'
                
                # 如果Alpaca volume为0或无效，尝试从Finnhub获取
                if final_volume == 0 or final_volume is None:
                    print(f'[Volume Fix] {symbol} Alpaca volume无效 ({final_volume})，尝试Finnhub')
                    ...
                    if finnhub_data and not finnhub_error:
                        ...
                        if finnhub_volume > 0:
                            ...
                            data_source = 'Alpaca+Finnhub (volume)'
        ...
        try:
            ...
            # 1. 首先检查Finnhub profile中的sector
            if profile_data.get('finnhubSector'):
                sector_info = profile_data.get('finnhubSector')
                sector_source = 'finnhub_profile'
                print(f'[Sector Info] {symbol}: 从Finnhub profile获取Sector: {sector_info}')
            elif profile_data.get('sector'):
                sector_info = profile_data.get('sector')
                sector_source = 'profile'
                print(f'[Sector Info] {symbol}: 从profile获取Sector: {sector_info}')
            else:
                # 2. 从备选来源获取Sector信息
                ...
                if sector_info and sector_info != 'Unknown':
                    ...
                    print(f'[Sector Info] {symbol}: 从备选来源推断Sector: {sector_info}')
                else:
                    # 3. 最后尝试DeepSeek推断
                    sector_info = infer_sector_with_deepseek(symbol, stock_data, news_data, profile_data)
                    if sector_info and sector_info != 'Unknown':
                        sector_source = 'deepseek_inferred'
                        print(f'[Sector Info] {symbol}: 从DeepSeek推断Sector: {sector_info}')
                    else:
                        sector_info = 'Unknown'
                        sector_source = 'unknown'
                        print(f'[Sector Info] {symbol}: 无法获取Sector信息')
...
def analyze_news_for_stock(symbol):
    ...
    try:
        # 获取新闻
        news_items, news_error = fetch_finnhub_company_news(symbol, days_back=7)
...
def analyze_trend_with_deepseek(symbol, stock_data, news_data, profile_data):
    ...
    try:
        ...
        if not api_key or api_key.startswith('sk-') and len(api_key) < 30:
            print(f'无有效的DeepSeek API密钥，使用本地规则分析 {symbol}')
        ...
        # 构建提示
        prompt = f"""作为专业的量化分析师，请分析以下股票并给出趋势判断：
  ...
  "scannerReason": "分析原因"
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                # 验证必要字段
                required_fields = ['trendLabel', 'trendScore', 'trendConfidence', 'scannerReason']
                for field in required_fields:
                ...
                print(f'解析DeepSeek响应失败: {str(e)}，使用本地分析')
                return analyze_trend_locally(symbol, stock_data, news_data, profile_data)
        else:
            print(f'DeepSeek API调用失败: {response.status_code}，使用本地分析')
            return analyze_trend_locally(symbol, stock_data, news_data, profile_data)
...
def analyze_trend_locally(symbol, stock_data, news_data, profile_data):
    ...
    try:
        ...
        # 新闻情绪因素
        sentiment = news_data.get('sentiment', 'Mixed')
        ...
        # 事件风险因素
        event_risk = news_data.get('eventRisk', 'Low')
        if event_risk == 'High':
            base_score -= 15
        ...
        reasons = []
        if change_pct > 2:
            reasons.append(f"Price up {change_pct:.1f}%")
        elif change_pct < -2:
            reasons.append(f"Price down {change_pct:.1f}%")
        ...
        if event_risk == 'High':
            ...
            reasons.append("Medium event risk")
        
        if volume > 10000000:
            reasons.append("High volume participation")
        
        scanner_reason = ", ".join(reasons) if reasons else "Neutral technical and news signals"
        # 明确标记为规则分析
        scanner_reason = f"Rule-based: {scanner_reason}"
...
def get_stock_detail(symbol):
    ...
    try:
        ...
        # AI Agent页面要求：必须使用Alpaca真实数据，不能回退到其他数据源
        if not alpaca_data:
            print(f'[股票详情] AI Agent页面要求: 必须使用Alpaca真实数据，但Alpaca数据获取失败')
            return jsonify({
                "symbol": symbol_upper,
