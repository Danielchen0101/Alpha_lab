...
# 优先使用 .env 文件中的配置，如果不存在则使用默认值
try:
    # 直接使用从config导入的变量
    alpaca_config_state = {
        'paper_api_key': ALPACA_API_KEY,  # 直接使用导入的变量
        'paper_api_secret': ALPACA_API_SECRET,  # 直接使用导入的变量
        'live_api_key': '',  # Live trading key - 预留，用于真实交易
        'live_api_secret': '',  # Live trading secret
        'environment': 'paper'  # 'paper' 或 'live' - 当前为 paper 环境
    }
    print(f"[Alpaca配置] 成功使用config.py中的Paper API Key: {alpaca_config_state['paper_api_key'][:10]}...")
except NameError as e:
    print(f"[Alpaca配置] 警告: 无法从config导入Alpaca变量: {e}")
    # 回退到默认值
    alpaca_config_state = {
        'paper_api_key': 'PKFQZZXERLVJLJHODHPPEB52RD',  # 默认Paper trading key
        'paper_api_secret': '5odo2jBF7YFLa7DAvss3hV7WVXE789ktTor7zMyPewxa',  # 默认Paper trading secret
        'live_api_key': '',  # Live trading key - 预留，用于真实交易
        'live_api_secret': '',  # Live trading secret
        'environment': 'paper'  # 'paper' 或 'live' - 当前为 paper 环境
    }
    print(f"[Alpaca配置] 使用默认Paper API Key: {alpaca_config_state['paper_api_key'][:10]}...")

# 打印 Alpaca 配置状态
print(f"[Alpaca配置] 环境: {alpaca_config_state['environment']}")
print(f"[Alpaca配置] Paper API Key前10位: {alpaca_config_state['paper_api_key'][:10]}...")
print(f"[Alpaca配置] Paper API Secret前10位: {alpaca_config_state['paper_api_secret'][:10]}...")
...
def fetch_alpaca_stock_data(symbol):
    ...
    try:
        ...
        if daily_bars:
            # 首先尝试最新的bar（可能是当天）
            latest_bar = daily_bars[-1] if len(daily_bars) > 0 else None
...
def fetch_alpaca_stock_data_snapshot(symbols):
    ...
    # 直接使用从config导入的Alpaca密钥（确保一致性）
    try:
        # 尝试使用全局变量中的Alpaca密钥
        api_key = ALPACA_API_KEY
        api_secret = ALPACA_API_SECRET
        print(f'[Alpaca数据] 使用config.py中的Alpaca密钥: {api_key[:10]}...')
    except NameError:
        # 回退到alpaca_config_state
        print(f'[Alpaca数据] 警告: 无法找到ALPACA_API_KEY变量，回退到alpaca_config_state')
        if environment == 'paper':
            api_key = alpaca_config_state.get('paper_api_key')
            api_secret = alpaca_config_state.get('paper_api_secret')
        else:
    ...
    if not api_key or not api_secret:
        ...
        print(f'[Alpaca数据] environment: {environment}, api_key: {api_key}, api_secret: {api_secret}')
    ...
    try:
        ...
        print(f'[Alpaca数据] 请求头: {market_headers}')
        print(f'[Alpaca数据] 使用环境: {environment}, API密钥: {api_key[:8]}...{api_key[-4:] if api_key else "None"}')
