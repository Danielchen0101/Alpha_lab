...
def test_dashboard_performance():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if errors:
                print(f"   ⚠️  错误股票: {len(errors)}只")
                for error_stock in errors[:3]:  # 只显示前3个错误
                    print(f"      {error_stock.get('symbol')}: {error_stock.get('error')}")
            
            # 检查超时情况
            if elapsed > 30:
                print(f"   ❌ 超时警告: 请求耗时超过30秒 ({elapsed:.2f}秒)")
            elif elapsed > 20:
                print(f"   ⚠️  性能警告: 请求耗时较长 ({elapsed:.2f}秒)")
            else:
                print(f"   ✅ 性能正常: 请求耗时合理 ({elapsed:.2f}秒)")
    ...
    for symbol in test_symbols:
        ...
        try:
            ...
            if response.status_code == 200:
                print(f"   {symbol}: ✅ {elapsed:.2f}秒")
            else:
                print(f"   {symbol}: ❌ {elapsed:.2f}秒 (状态码: {response.status_code})")
                
        except Exception as e:
            elapsed = time.time() - start_time
            print(f"   {symbol}: ❌ {elapsed:.2f}秒 (异常: {str(e)[:50]})")
    
    # 测试3: 测试并发性能（使用Finnhub服务）
    print("\n3. 测试Finnhub服务性能（并发）")
    print("-" * 40)
    ...
    try:
        ...
        print(f"   测试{len(symbols)}只股票的并发获取...")
        
        # 测试轻量级模式
        start_time = time.time()
        result = finnhub_service.get_multiple_stocks(symbols, lightweight=True)
        elapsed = time.time() - start_time
        
        print(f"   Finnhub轻量级模式: {elapsed:.2f}秒")
        print(f"   成功: {result.get('count', 0)}只")
        ...
        # 测试完整模式
        start_time = time.time()
        result = finnhub_service.get_multiple_stocks(symbols[:5], lightweight=False)  # 只测试5只
        elapsed = time.time() - start_time
        
        print(f"   Finnhub完整模式(5只): {elapsed:.2f}秒")
        
    except Exception as e:
        print(f"   ❌ Finnhub测试失败: {str(e)}")
