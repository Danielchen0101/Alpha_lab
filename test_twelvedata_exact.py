...
print("=== 测试Twelve Data API（使用后端API密钥）===")
...
if response.status_code == 200:
    ...
    if 'values' in data:
        ...
        if values:
            ...
            # 检查是否包含今天
            has_today = any('2026-03-20' in item.get('datetime', '') for item in values)
            print(f"包含今天(3/20)数据: {has_today}")
            
            if has_today:
                today_data = [item for item in values if '2026-03-20' in item.get('datetime', '')]
...
print("\n=== 关键问题 ===")
print("如果Twelve Data返回300个点且有:00和:30，但后端只返回35个点")
print("那么问题一定在后端的数据处理逻辑中")